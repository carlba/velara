...
export default function ProfilePage() {
  ...
  const handleTraktZipFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;
    ...
    <div className="space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-muted-foreground">
          Import movie and TV show ratings, watch history, or comments from Filmtipset and Trakt
          to your account.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Paste your Filmtipset CSV below or choose a file to load, or upload a Trakt data-dump
            zip. The Trakt import will create movie and TV show ratings and watch history; the
            Filmtipset import will create ratings, watch entries, and comments where supported.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {provider === 'filmtipset' ? (
            ...
            <div className="grid gap-2">
              <Label htmlFor="trakt-zip-file">Trakt data dump (.zip)</Label>
              <Input
                id="trakt-zip-file"
                type="file"
                accept=".zip,application/zip"
                ref={fileInputRef}
                onChange={handleTraktZipFileChange}
              />
            </div>
          )}
